...
var $author$project$Pages$Login$Model = F4(
	function (user, key, email, password) {
		return {email: email, key: key, password: password, user: user};
	});
	...
	return $elm$http$Http$post(
		{
			...
			expect: A2(
				...
				options.onResponse,
				A2($elm$json$Json$Decode$field, 'user', $author$project$Api$User$decoder)),
			url: $author$project$Api$User$loginUrl
		});
};
...
var $author$project$Pages$Projects$fetchProjects = function (model) {
	...
	function (shared, _v0) {
		...
		return _Utils_Tuple2(
			...
			$elm$core$Platform$Cmd$batch(
				_List_fromArray(
					[
						$author$project$Pages$Projects$fetchProjects(model)
					])));
	});
